...
#include <beman/task/detail/task_promise_base.hpp>
...
template <class Ret, class... Queries> class basic_task {
  ...
  template <class Receiver> class operation {
   public:
    ...
    explicit operation(::std::coroutine_handle<promise_type> handle, Receiver receiver) noexcept
        : receiver_{receiver}
        , handle_{handle} {
      handle_.promise().set_receiver(receiver, stop_source_.get_token());
    }
 ...
 public:
  ...
  template <class Receiver> auto connect(Receiver receiver) && noexcept -> operation<Receiver> {
    return operation<Receiver>{handle_, ::std::move(receiver)};
  }
...
class basic_task<Ret, Queries...>::promise_type
    : public ::beman::task::detail::task_promise_base<Ret, promise_type, Queries...> {
 public:
    ...
    void await_suspend(::std::coroutine_handle<promise_type> h) noexcept {
      ...
      try {
        if constexpr (::std::is_void_v<Ret>) {
          ::std::move(promise).get_result();
          ::beman::execution26::set_value(::std::move(promise.receiver_ref_));
        } else {
          ::beman::execution26::set_value(::std::move(promise.receiver_ref_),
                                          ::std::move(promise).get_result());
        }
      } catch (...) {
        ::beman::execution26::set_error(::std::move(promise.receiver_ref_),
